...
DASHReceiver::DASHReceiver          (IMPD *mpd, IDASHReceiverObserver *obs, MediaObjectBuffer *buffer, uint32_t bufferSize, int k) :
              mpd                   (mpd),
              period                (NULL),
              adaptationSet         (NULL),
              representation        (NULL),
...
void DASHReceiver::setK(int pushnr)
{
    this->k = pushnr;
}

int DASHReceiver::getK()
{
    return this->k;
}
...
MediaObject*                DASHReceiver::GetNextSegment            ()
{
    ...
    std::cout << "[DASHReceiver] Get the segment for " << segmentNumber + segmentOffset << std::endl;
    if(k > 0){
        if(this->segmentNumber + this->segmentOffset >= this->beacon){
            printf("[DASHReceiver] Reach the beacon %d (%d), start a new push cycle.\n", this->beacon, this->segmentOffset + this->segmentNumber);
            this->beacon += (this->k + 1);
            this->pushRepresentation = this->representation;
            this->pushRepresentationStream = this->representationStream;
        }
    }

    if(k > 0){
        if(this->segmentNumber >= this->pushRepresentationStream->GetSize())
            return NULL;
    }else{
    ...
    if(k > 0){
        seg = this->pushRepresentationStream->GetMediaSegment(this->segmentNumber + this->segmentOffset);
    }else{
    ...
    if (seg != NULL)
    {
        MediaObject *media;
        if(k > 0){
            media = new MediaObject(seg, this->pushRepresentation, k);
        }else{
            media = new MediaObject(seg, this->representation);
        }
        printf("[DASHReceiver] Get segment for url: %s\n", media->AbsoluteURI().c_str());
        this->segmentNumber++;
        return media;
    }
...
uint32_t                    DASHReceiver::CalculateSegmentOffset    ()
{
    ...
    std::cout << "[DASHReceiver] firstSegnum: " << firstSegNum << ", currSegNum:" << currSegNum << ", startSegNum:" << startSegNum << std::endl;
