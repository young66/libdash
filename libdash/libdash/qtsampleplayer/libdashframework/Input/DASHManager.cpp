...
DASHManager::DASHManager        (uint32_t maxCapacity, IDASHManagerObserver* stream, IMPD* mpd, int k) :
             readSegmentCount   (0),
             receiver           (NULL),
             ...
             k                  (k)
{
    ...
    this->receiver  = new DASHReceiver(mpd, this, this->buffer, maxCapacity, k);
}
...
void DASHManager::setK(int pushnr)
{
    this->k = pushnr;
    if(this->receiver)
        this->receiver->setK(pushnr);
}
int DASHManager::getK()
{
    return this->k;
}
