...
void            QtSamplePlayerGui::SetGuiFields                                     (dash::mpd::IMPD* mpd)
{
    ...
    this->SetKComboBox(10, this->ui->cb_k);
...
void QtSamplePlayerGui::SetKComboBox (int k, QComboBox *cb)
{
    cb->clear();
...
/* Notifiers */
void QtSamplePlayerGui::NotifyKChanged ()
{
    this->LockUI();
    int k                   = this->ui->cb_k->currentIndex();

    for(size_t i = 0; i < this->observers.size(); i++)
        this->observers.at(i)->OnKChanged(k);
...
void            QtSamplePlayerGui::on_cb_k_currentIndexChanged                 (int index)
{
    ...
    std::cout << "cb_k index changes to " << index << std::endl;
    this->NotifyKChanged();
