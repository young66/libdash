...
MultimediaStream::MultimediaStream  (StreamType type, IMPD *mpd, uint32_t bufferSize, uint32_t frameBufferSize, uint32_t sampleBufferSize, int k) :
                  type              (type),
...
void MultimediaStream::setK(int pushnr)
{
    this->k = pushnr;
    if(this->dashManager)
        this->dashManager->setK(pushnr);
}

int MultimediaStream::getK()
{
    return this->k;
}
...
{
    this->dashManager   = new DASHManager(this->segmentBufferSize, this, this->mpd, this->k);
