...
MultimediaManager::MultimediaManager            (QTGLRenderer *videoElement, QTAudioRenderer *audioElement, int k) :
...
void MultimediaManager::setK(int pushnr)
{
    this->k = pushnr;
    if(this->videoStream)
        this->videoStream->setK(pushnr);
}

int MultimediaManager::getK()
{
    return this->k;
}
...
void    MultimediaManager::InitVideoRendering               (uint32_t offset)
{
    ...
    this->videoStream = new MultimediaStream(sampleplayer::managers::VIDEO, this->mpd, SEGMENTBUFFER_SIZE, 2, 0, this->k);
