...
CurlmManager AbstractChunk::curlmmanager = CurlmManager();
AbstractChunk::AbstractChunk        ()  :
               connection           (NULL),
               dlThread             (NULL),
               bytesDownloaded      (0)
{
    AbstractChunk::curlmmanager.startFetching();
}
AbstractChunk::~AbstractChunk       ()
{
    this->AbortDownload();
...
bool    AbstractChunk::StartDownload                ()
{
    ...
    AbstractChunk::curlmmanager.commitHttpTransaction(this->AbsoluteURI(), {}, CurlResponseCallback, CurlCompleteCallback, (void *)this);

    this->stateManager.State(IN_PROGRESS);

    return true;
}
bool    AbstractChunk::StartDownload                (std::vector<std::string> headers)
{
    if(this->stateManager.State() != NOT_STARTED)
        return false;

    //sprintf(pushDirective, "PushDirective: urn:mpeg:dash:fdh:2016:push-next;%d", 2);
    AbstractChunk::curlmmanager.commitHttpTransaction(this->AbsoluteURI(), headers, CurlResponseCallback, CurlCompleteCallback, (void *)this);
...
void    AbstractChunk::CurlCompleteCallback (void *abstractchunk)
{
    AbstractChunk *chunk  = (AbstractChunk *) abstractchunk;

    printf("[AbstractChunk] Download %s completed!\n", chunk->AbsoluteURI().c_str());
...
size_t  AbstractChunk::CurlResponseCallback         (void *contents, size_t size, size_t nmemb, void *userp)
{
    ...
    printf("[AbstractChunk] Downloading %s...(%ld bytes received)\n", chunk->AbsoluteURI().c_str(), realsize);
